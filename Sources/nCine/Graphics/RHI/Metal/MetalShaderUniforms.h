...
namespace nCine::RHI::Metal
{
	...
	/**
		...
		Owns a @ref MetalUniformCache for every active uniform of a program that is not part of a block,
	...
	*/
	...
	public:
		...
		// inside each cache's uniform), so range-iteration yields `MetalUniformCache&` directly — matching
		// the OpenGL backend's `GetAllUniforms()` iteration semantics that the render batcher relies on.
		using UniformHashMapType = std::vector<MetalUniformCache>;

		MetalShaderUniforms();
		explicit MetalShaderUniforms(MetalShaderProgram* shaderProgram);
		MetalShaderUniforms(MetalShaderProgram* shaderProgram, const char* includeOnly, const char* exclude);

		inline void SetProgram(MetalShaderProgram* shaderProgram) {
			SetProgram(shaderProgram, nullptr, nullptr);
		}
		void SetProgram(MetalShaderProgram* shaderProgram, const char* includeOnly, const char* exclude);
		...
		/** @brief Returns the container of all managed uniform caches (range-iterates as `MetalUniformCache&`) */
	...
	private:
		MetalShaderProgram* _shaderProgram;
		bool _maybeDirty;
	...
	/**
		...
		Owns a @ref MetalUniformBlockCache per active block, distributes a shared host data buffer across
		them, copies the block contents into a suballocated range of the streaming uniform buffer on @ref
		CommitUniformBlocks(), and on @ref Bind() forwards each range to the device (which copies it into its per-frame `MTL::Buffer` ring at draw time).
	*/
	...
	public:
		...
		// block), so range-iteration yields `MetalUniformBlockCache&` directly, matching the OpenGL backend.
		using UniformHashMapType = std::vector<MetalUniformBlockCache>;
		...
		explicit MetalShaderUniformBlocks(MetalShaderProgram* shaderProgram);
		MetalShaderUniformBlocks(MetalShaderProgram* shaderProgram, const char* includeOnly, const char* exclude);
		...
		/** @brief Returns the container of all managed block caches (range-iterates as `MetalUniformBlockCache&`) */
