...
namespace nCine::RHI::Metal
{
	...
	/**
		...
		MetalUniformCache holds the value.
	*/
	class MetalUniform
	{
		friend class MetalShaderProgram;
		friend class MetalUniformBlock;
	...
	public:
		...
		MetalUniform(MetalShaderProgram* owner, const char* name, ShaderCompiler::UniformType type, std::int32_t arraySize, std::int32_t location)
		...
		inline MetalShaderProgram* GetOwner() const {
			return _owner;
		}
	...
	*/
	...
	{
		friend class MetalShaderProgram;
		friend class MetalUniformBlockCache;
	...
	public:
		...
		MetalUniformBlock(std::uint32_t index, const char* name, std::int32_t dataSize)
		...
		MetalUniform* GetUniform(const char* name) {
			for (MetalUniform& u : _members) {
	...
	private:
		...
		std::vector<MetalUniform> _members;
	...
	*/
	...
	public:
		...
		MetalAttribute(const char* name, ShaderCompiler::UniformType type, std::int32_t location)
