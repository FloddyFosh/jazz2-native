...
namespace nCine::RHI::Metal
{
	...
	/**
		@brief Vertex layout description of the Metal backend (aliased as `RHI::VertexFormat`)
		...
		builds the render pipeline's `MTL::VertexDescriptor` from the recorded layout (fed by the offline
		shader reflection).
	*/
	...
	public:
		...
		class Attribute
		{
			friend class MetalVertexFormat;
		...
		public:
			...
			// and avoids needing the complete `MetalBufferObject` type in this header
			...
			inline const MetalBufferObject* GetVbo() const {
				return _vbo;
			}
			...
			inline void setVbo(const MetalBufferObject* vbo) {
				_vbo = vbo;
			}
		...
		inline const MetalBufferObject* GetIbo() const {
			return _ibo;
		}
		inline void SetIbo(const MetalBufferObject* ibo) {
			_ibo = ibo;
		}
		...
		bool operator==(const MetalVertexFormat& other) const {
	...
	private:
		...
		const MetalBufferObject* _ibo;
	};

	/**
		@brief Vertex array object stub of the Metal backend (aliased as `RHI::VertexArray`)

		Metal has no vertex-array object; the vertex input state lives in the render pipeline and the
		buffer binding is set per draw with `setVertexBuffer()`. This only satisfies the contract alias
