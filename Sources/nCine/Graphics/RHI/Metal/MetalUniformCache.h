...
namespace nCine::RHI::Metal
{
	/**
		...
		committing is a no-op — the whole block is published by @ref MetalShaderUniformBlocks.
	*/
	...
	public:
		...
		explicit MetalUniformCache(const MetalUniform* uniform)
			: _uniform(uniform), _dataPointer(nullptr), _isDirty(false) {}

		inline const MetalUniform* GetUniform() const {
			return _uniform;
		}
	...
	/**
		...
		MetalUniformCache for each member, each pointing at its member offset within that buffer. The block is
		uploaded as one contiguous range by @ref MetalShaderUniformBlocks.
	*/
	class MetalUniformBlockCache
	{
	public:
		MetalUniformBlockCache()
			: _uniformBlock(nullptr), _dataPointer(nullptr), _usedSize(0) {}
		explicit MetalUniformBlockCache(MetalUniformBlock* uniformBlock);

		inline const MetalUniformBlock* uniformBlock() const {
		...
		MetalUniformCache* GetUniform(StringView name);
		void SetBlockBinding(std::int32_t blockBinding);

	private:
		MetalUniformBlock* _uniformBlock;
		std::uint8_t* _dataPointer;
