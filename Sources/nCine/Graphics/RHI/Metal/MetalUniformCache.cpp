...
namespace nCine::RHI::Metal
{
	...
	bool MetalUniformCache::SetFloatValue(float v0)
	{
	...
	bool MetalUniformCache::SetFloatValue(float v0, float v1)
	{
	...
	bool MetalUniformCache::SetFloatValue(float v0, float v1, float v2)
	{
	...
	bool MetalUniformCache::SetFloatValue(float v0, float v1, float v2, float v3)
	{
	...
	bool MetalUniformCache::SetIntVector(const std::int32_t* vec)
	{
	...
	bool MetalUniformCache::SetIntValue(std::int32_t v0)
	{
	...
	bool MetalUniformCache::SetIntValue(std::int32_t v0, std::int32_t v1)
	{
	...
	bool MetalUniformCache::SetIntValue(std::int32_t v0, std::int32_t v1, std::int32_t v2)
	{
	...
	bool MetalUniformCache::SetIntValue(std::int32_t v0, std::int32_t v1, std::int32_t v2, std::int32_t v3)
	{
	...
	bool MetalUniformCache::CommitValue()
	{
	...
	bool MetalUniformCache::CheckFloat() const
	{
		return (_uniform != nullptr && _uniform->IsFloat());
	}

	bool MetalUniformCache::CheckInt() const
	{
		return (_uniform != nullptr && !_uniform->IsFloat());
	}

	bool MetalUniformCache::CheckComponents(std::uint32_t requiredComponents) const
	{
	...
	MetalUniformBlockCache::MetalUniformBlockCache(MetalUniformBlock* uniformBlock)
		: _uniformBlock(uniformBlock), _dataPointer(nullptr), _usedSize(0)
	{
		...
		for (MetalUniform& member : uniformBlock->_members) {
			_uniformCaches.push_back({member.GetName(), MetalUniformCache(&member)});
		}
	}

	std::uint32_t MetalUniformBlockCache::GetIndex() const
	{
		return (_uniformBlock != nullptr ? _uniformBlock->GetIndex() : 0);
	}

	std::int32_t MetalUniformBlockCache::GetBindingIndex() const
	{
		return (_uniformBlock != nullptr ? _uniformBlock->GetBindingIndex() : 0);
	}

	std::int32_t MetalUniformBlockCache::GetSize() const
	{
		return (_uniformBlock != nullptr ? _uniformBlock->GetSize() : 0);
	}

	std::uint8_t MetalUniformBlockCache::GetAlignAmount() const
	{
		return (_uniformBlock != nullptr ? _uniformBlock->GetAlignAmount() : 0);
	}

	void MetalUniformBlockCache::SetDataPointer(std::uint8_t* dataPointer)
	{
	...
	void MetalUniformBlockCache::SetUsedSize(std::int32_t usedSize)
	{
		if (usedSize >= 0) {
			_usedSize = usedSize;
		}
	...
	bool MetalUniformBlockCache::CopyData(std::uint32_t destIndex, const std::uint8_t* src, std::uint32_t numBytes)
	{
	...
	MetalUniformCache* MetalUniformBlockCache::GetUniform(StringView name)
	{
		for (NamedCache& named : _uniformCaches) {
			if (name == named.Name) {
				return &named.Cache;
	...
	void MetalUniformBlockCache::SetBlockBinding(std::int32_t blockBinding)
	{
