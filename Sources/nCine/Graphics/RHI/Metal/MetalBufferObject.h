...
{
	/**
		@brief Buffer object of the Metal backend (aliased as `RHI::Buffer`)
		...
		sub-range to the device. It additionally backs the Vertex/Index targets with a real shared-storage
		`MTL::Buffer` (@ref GetMtlBuffer()), refreshed from the host store when dirty - the device binds it
		with `setVertexBuffer()` / passes it to `drawIndexedPrimitives()`. Uniform-target buffers stay
		host-only: their bound ranges are copied into the device's per-frame uniform ring at draw time instead.
	*/
	class MetalBufferObject
	{
	public:
		explicit MetalBufferObject(BufferTarget target);
		~MetalBufferObject();

		MetalBufferObject(const MetalBufferObject&) = delete;
		MetalBufferObject& operator=(const MetalBufferObject&) = delete;

		/** @brief Returns the `MTL::Buffer*` (as `void*`) mirroring the host store (Vertex/Index targets), refreshing it if dirty; `nullptr` for Uniform targets or before any data */
		...
		/** @brief Flushes a mapped range (marks the GPU mirror dirty) */
		void FlushMappedBufferRange(std::size_t offset, std::size_t length);
		/** @brief Unmaps the buffer (marks the GPU mirror dirty) */
		bool Unmap();
	...
	private:
		...
		// Shared-storage GPU buffer mirroring the host store for the Vertex/Index targets (Uniform targets stay
		// host-only). Kept as an opaque pointer so this contract header does not need metal-cpp; the .cpp casts
		// it to MTL::Buffer*. Refreshed lazily from the store when _gpuDirty is set.
		mutable void* _gpuBuffer;
