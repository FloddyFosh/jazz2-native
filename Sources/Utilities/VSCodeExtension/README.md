...
- the compile-time macros `VERTEX_STAGE`, `FRAGMENT_STAGE`, `SOFTWARE_RENDERER`,
  `NO_DYNAMIC_BRANCHING`, `LOW_POWER_GPU` — in `#ifdef` / `#ifndef` and in `#if` / `#elif` expressions, where one
...
| inside `void fixed_function(…)`'s parentheses | `pvr`, `gx`, `pica`, `gu`, `gs`, `rdp`, `legacygl` |
...
| Show Metal Shading Language Transform | `--msl` |
